...
export default function EditTrackApplication({ isEditOpen, onClose, onSave, application }) {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    _id: "", company: "", role: "", location: "", date: "", status: "Applied", link: "",
  });
            ...
            <div className="relative p-6 sm:p-7">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2.5">
                  ...
                  <h2 className="text-base font-bold text-color">Edit Application</h2>
                </div>
                <button
                  onClick={onClose}
                ...
                <div className="space-y-4">
                  <FieldGroup label="Company" icon={Briefcase}>
                    <input type="text" name="company" value={formData.company} onChange={handleChange}
                      placeholder="e.g. Google" className="modal-input" />
                  </FieldGroup>

                  <FieldGroup label="Role" icon={Tag}>
                    <input type="text" name="role" value={formData.role} onChange={handleChange}
                      placeholder="e.g. Frontend Engineer" className="modal-input" />
                  </FieldGroup>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <FieldGroup label="Location" icon={MapPin}>
                      <input type="text" name="location" value={formData.location} onChange={handleChange}
                        placeholder="e.g. Remote" className="modal-input" />
                    </FieldGroup>
                  ...
                  <FieldGroup label="Job Link (Optional)" icon={Link2}>
                    <input type="text" name="link" value={formData.link} onChange={handleChange}
                      placeholder="https://..." className="modal-input" />
                  </FieldGroup>
